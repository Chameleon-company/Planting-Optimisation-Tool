...
describe("AhpComparison", () => {
    ...
    it("calls onCancel when cancel button is clicked", async () => {
        ...
        render(
            <AhpComparison factors={["A", "B"]} speciesName="Test" onComplete={vi.fn()} onCancel={onCancel} />
        );
...
describe("AhpResultsTable", () => {
    ...
    it("shows acceptable message and Reset button if consistent", () => {
        render(
            <AhpResultsTable data={mockData} speciesName="Test" onReset={vi.fn()} onRetry={vi.fn()} />
        );

        expect(screen.getByText(/Acceptable/i)).toBeInTheDocument();
        expect(screen.getByRole("button", { name: /Profile Another Species/i })).toBeInTheDocument();
    });

    it("shows error message and Retry button if inconsistent", () => {
        const badData = { ...mockData, is_consistent: false, consistency_ratio: 0.25, message: "Inconsistent results" };

        render(
            <AhpResultsTable data={badData} speciesName="Test" onReset={vi.fn()} onRetry={vi.fn()} />
        );

        expect(screen.getByText(/Inconsistent/i)).toBeInTheDocument();
        expect(screen.getByRole("button", { name: /Profile Again/i })).toBeInTheDocument();
    });
});