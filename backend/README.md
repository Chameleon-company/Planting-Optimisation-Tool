...
## DEM Data Setup (PostGIS)

To enable sapling estimation, a DEM (Digital Elevation Model) must be loaded into the database.
The standard migrations do not include DEM data, so the following manual steps are required.
...
### 3. Load DEM Data using raster2pgsql

Use the raster2pgsql tool to import a `.tif` DEM file:

```bash
raster2pgsql -I -C -M path/to/DEM.tif public.dem_table | psql -U postgres -d POT_db
```

Example:

```bash
raster2pgsql -I -C -M ./data/DEM.tif public.dem_table | psql -U postgres -d POT_db
```
...
```sql
SELECT rid, ST_Width(rast), ST_Height(rast) FROM dem_table;
```
...
* DEM data is required for slope calculation and sapling estimation.
* The service supports multi-tile DEM inputs using `ST_Union`.
* For testing purposes, a lightweight execution path is used to avoid heavy raster operations.
