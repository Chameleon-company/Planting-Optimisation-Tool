...
def compute_farm_slope(DEM_path: str, farm_boundary_path: str, farm_lon: float, farm_lat: float, output_path: str):
    ...
    if all_farm_boundaries.crs is None:                      # Prints an error if file does not have a CRS
        raise ValueError("ERROR: Farm boundary data has no CRS, please check farm boundary file.")
    ...
    farm_point = gpd.GeoSeries([Point(farm_lon, farm_lat)], crs="EPSG:4326").to_crs(all_farm_boundaries.crs)
    
    # Finds polygon that contains the point
    polygon_in_point = all_farm_boundaries[all_farm_boundaries.geometry.contains(farm_point.iloc[0])]
    ...
    farm_polygon = polygon_in_point.geometry.iloc[0]                           # Extract geometry of the matched farm polygon
    ...
    with rasterio.open(DEM_path) as src:
        ...
        farm_poly_dem = gpd.GeoSeries([farm_polygon], crs=all_farm_boundaries.crs).to_crs(DEM_crs).iloc[0]  # Reprojects polygon to DEM CRS
        ...
        profile.update(height=slope.shape[0], width=slope.shape[1], transform=dem_transform, dtype=rasterio.float32, count=1)
        slope = slope.astype(np.float32)

    # Save slope raster
    with rasterio.open(output_path, "w", **profile) as dest:  # Save updated data into output file path
...
def slope_tester(farm_DEM_path: str):
    with rasterio.open(farm_DEM_path) as src:
        slope = src.read(1)                    # Read data in first layer of slope file & write into array 'slope'
    valid = True
